...
def test_TastFilter_custom_level(tasks_data):
    filter_set = TaskFilter({"custom_level": 5}, queryset=Task.objects.all())
...
def test_TaskFilter_get_ordering(tasks_data):
    filter_set_desc = TaskFilter(
        {"ordering": "publicated_at"}, queryset=Task.objects.all()
    )
    filter_set_asc = TaskFilter(
        {"ordering": "publicated_at"}, queryset=Task.objects.all()
    )
    filter_desc_result = filter_set_desc.qs
    filter_asc_result = filter_set_asc.qs
    ...
    assert list(filter_asc_result) == list(filter_test_asc)
