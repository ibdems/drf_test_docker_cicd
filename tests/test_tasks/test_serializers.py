...
class TaskSerializerTestCase(TestCase):
    ...
    def test_ckeck_date_time(self):
        ...
        serializer = TaskSerializer(data=self.data)
        self.assertEqual(serializer.is_valid(), False)
        self.assertIn("la date ne doit pas etre dans le passe", serializer.errors['created_at'])
        self.assertIn("la date ne doit pas etre dans le passe", serializer.errors['expired_at'])
