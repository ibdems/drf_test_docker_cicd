...
class TaskSerializerTestCase(TestCase):
    ...
    def test_validate_name_with_invalid_data(self):
        ...
        self.assertEqual(serializer.errors["name"][0], "Nom de tache unique")
    ...
    def test_ckeck_date_time(self):
        self.data["created_at"] = timezone.now() + timedelta(days=-2)
        self.data["expired_at"] = timezone.now() + timedelta(days=-1)
        serializer = TaskSerializer(data=self.data)
        self.assertEqual(serializer.is_valid(), False)
        self.assertIn(
            "la date ne doit pas etre dans le passe", serializer.errors["created_at"]
        )
        self.assertIn(
            "la date ne doit pas etre dans le passe", serializer.errors["expired_at"]
        )
